...
# V15.7.0 — UI sound polish + SVG licence marks

V15.7.0 adds lightweight interface feedback while keeping the convection simulation itself unchanged. **Physics is unchanged.**

## Sound design

The sound system is entirely event-driven and has no continuous background audio or per-frame processing.

- `assets/ui-hover.wav`: subtle ~55 ms hover cue, used only on devices with a fine pointer and real hover capability.
- `assets/ui-click.wav`: subtle ~90 ms tactile cue for button presses/taps.
- `assets/heater-gurgle.wav`: approximately 3 seconds, played only when the heater changes **Off → On** and when **Tutorial starts**.
- One Sound toggle controls the UI cues and heater gurgle together.
- Audio is enabled by default, but browser autoplay rules mean hover audio naturally begins only after the user has interacted with the page.
- The one-time pedagogical notice remains: the gurgle is decorative and convection itself does not produce bubbling sounds.

The implementation uses tiny preloaded `HTMLAudioElement` pools rather than an AudioContext graph. There is no flow sampling, filter automation or animation-loop audio work.

## Licence artwork

The three CC/BY/NC raster PNGs were replaced with small padded SVG assets:

- `assets/cc-cc.svg`
- `assets/cc-by.svg`
- `assets/cc-nc.svg`

The SVGs stay sharp at any scale, avoid the previous clipped NC circle, and have a very small file footprint. The title marks are aligned to the title baseline so their lower edges sit with the bottom of `3D Convection`.

## Physics

No changes to the Boussinesq solver, particles, thermal expansion representation, packing, flow arrows, traces, equal-volume comparison, tutorial snapshot/reset behaviour, heater movement or camera behaviour.

# V15.7.1 — heater-movement sound + final SVG alignment

V15.7.1 removes the generic button hover/click sounds from V15.7.0. **Physics is unchanged.**

## Sound behaviour

The Sound control now affects heater-related effects only:

- the approximately 3-second gurgle still plays when the heater changes **Off → On**;
- the same gurgle plays once at the **start of Tutorial**;
- a soft glide/swipe sound plays while the heater is directly dragged in the 3D scene;
- the glide/swipe sound also plays while the heater-position range slider is being swiped;
- ordinary buttons, toolbar controls, hover states and taps/clicks are silent.

The movement sound is event-driven and stops on pointer release/cancel. There is no continuous ambience, no flow sampling and no per-frame sound work.

## Licence icons

The CC, BY and NC artwork remains in SVG format. A final CSS override aligns the lower edge of the three title icons with the bottom of the `3D Convection` title, matching the V15.6.6 treatment.

## Physics

No changes to the Boussinesq solver, particles, packing, heating, arrows, trace behaviour, comparison volumes, camera, tutorial state restoration or heater dynamics.
