// V15.7.0 classroom presentation refinement + final aquarium audio
...
// V15.7.0 lightweight UI + heater sound system -----------------------------
// Event-driven only: no continuous ambience, no flow sampling and no audio
// work in the animation loop. The applet uses three small local WAV assets:
//   - UI hover: subtle 55 ms tick (fine-pointer devices only)
//   - UI click: subtle 90 ms tactile click
//   - heater gurgle: ~3 s attention cue at heater Off -> On and Tutorial start
//
// A single Sound toggle controls all three. Audio is enabled by default.
...
const UI_HOVER_URL = 'assets/ui-hover.wav';
const UI_CLICK_URL = 'assets/ui-click.wav';
const finePointerHover = window.matchMedia?.('(hover: hover) and (pointer: fine)').matches ?? false;

function makeAudioPool(url, size, volume) {
  const pool = [];
  ...
  return function play() {
    ...
    promise?.catch?.(() => {}); // harmless before the browser's first gesture
    return true;
  };
}

const playUiHover = makeAudioPool(UI_HOVER_URL, 2, 0.30);
const playUiClick = makeAudioPool(UI_CLICK_URL, 3, 0.42);
...
function syncSoundUi() {
  ...
  soundToggleBtn.setAttribute(
    'aria-label',
    soundEnabled ? 'Turn off UI sounds and heater sound' : 'Turn on UI sounds and heater sound'
  );
  soundToggleBtn.title = soundEnabled
    ? 'Sound effects on — subtle UI feedback + heater effect'
    : 'Sound effects off';
}

function setSoundEnabled(enabled) {
  soundEnabled = Boolean(enabled);
  if (!soundEnabled) stopAllSound();
  syncSoundUi();
}

function isUiSoundButton(element) {
  return element instanceof Element && element.closest('button:not(:disabled)');
}

// Fine-pointer hover only; touch devices never receive hover sounds.
document.addEventListener('pointerover', event => {
  if (!finePointerHover || !soundEnabled) return;
...
// Pointer-down gives more immediate tactile feedback than waiting for click.
