// V15.4 compact teaching interface ---------------------------------------------------
...
const walkthroughSteps = [
  ...
  {
    title: 'Exam-ready summary · 4 marks',
    text: '1. Particles near the heater gain thermal energy / kinetic energy and move faster.\n2. The particles move further apart; the heated liquid expands and becomes less dense.\n3. The less-dense warm liquid rises while cooler, denser liquid sinks.\n4. The continuous circulation forms a convection current.',
    view: walkthroughFrontView,
    overlay: 'none', flow: true, trace: false,
  },
];
...
function isPresentationFullscreen() {
  return document.fullscreenElement === scenePanel ||
    document.body.classList.contains('presentation-mode');
}
...
function syncFullscreenUi() {
  ...
  // ResizeObserver normally catches this, but an explicit resize makes the
  // transition immediate on browsers that delay the observer callback.
  requestAnimationFrame(() => resizeRenderer());
}

async function enterPresentationFullscreen() {
  if (scenePanel.requestFullscreen) {
    try {
      await scenePanel.requestFullscreen();
      return;
    } catch (error) {
      console.warn('Fullscreen API unavailable; using presentation fallback.', error);
    }
  }
...
async function exitPresentationFullscreen() {
  if (document.fullscreenElement && document.exitFullscreen) {
    try {
      ...
      console.warn('Could not exit Fullscreen API cleanly.', error);
    }
  }
...
fullscreenBtn.addEventListener('click', async () => {
  if (isPresentationFullscreen()) await exitPresentationFullscreen();
  else await enterPresentationFullscreen();
});
...
walkthroughBtn.addEventListener('click', () => walkthroughActive ? finishWalkthrough(true) : startWalkthrough());
