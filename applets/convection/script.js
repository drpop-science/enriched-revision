// V15.6.1 classroom presentation refinement + water-like flow sonification
...
// V15.6.1 procedural flow sonification ---------------------------------------
// This remains a SONIFICATION of the simulation rather than literal molecular
// sound. For Year 9/10 teaching the texture is intentionally exaggerated so
// changes in the convection current are easy to hear.
//
// Design: pink-ish locally generated noise + three filtered paths:
//   1) broad moving-water body,
//   2) brighter stream texture,
//   3) gently pulsing resonant "burble" driven by vertical circulation.
// No prerecorded water, fish-tank, boiling or bubbling sample is used.
const AudioContextClass = window.AudioContext || window.webkitAudioContext;
let soundEnabled = false;
let soundEngine = null;
let lastSonificationMeasure = 0;
let sonificationSuspendTimer = null;
const SONIFICATION_MEASURE_MS = 80;

const SOUND_TUNING = Object.freeze({
  maxMasterGain: 0.18,
  stereoWidth: 0.42,
  bodyGain: 0.84,
  baseTextureGain: 0.28,
  baseBurbleGain: 0.035,
});
...
function makeFlowNoiseBuffer(ctx) {
  ...
  // Paul-Kellet-style pink-noise shaping: substantially less "radio hiss"
  // than white noise, while retaining enough mid-frequency detail for iPad
  // and laptop speakers.
  ...
  for (let i = 0; i < length; i++) {
    ...
    b6 = white*0.115926;
    data[i] = THREE.MathUtils.clamp(pink * 0.115, -1, 1);
  }

  // Loop crossfade to avoid a click every five seconds.
...
function ensureSoundEngine() {
  ...
  source.buffer = makeFlowNoiseBuffer(ctx);
  source.loop = true;

  // Moving-water body: remove sub-bass that tiny device speakers cannot
  // reproduce, then keep a broad low/mid wash.
  const bodyHighpass = ctx.createBiquadFilter();
  bodyHighpass.type = 'highpass';
  bodyHighpass.frequency.value = 135;
  ...
  lowGain.gain.value = SOUND_TUNING.bodyGain;

  // Stream texture: audible on iPad/laptop speakers without becoming hissy.
  ...
  textureGain.gain.value = SOUND_TUNING.baseTextureGain;

  // Resonant burble: amplitude/frequency are changed from the ACTUAL vertical
  // flow measurement, producing a watery gurgle without pretending the model
  // is physically boiling.
  const burbleFilter = ctx.createBiquadFilter();
  burbleFilter.type = 'bandpass';
  burbleFilter.frequency.value = 430;
  burbleFilter.Q.value = 4.5;

  const burbleGain = ctx.createGain();
  burbleGain.gain.value = SOUND_TUNING.baseBurbleGain;
  ...
  // Safety net: permits a much more useful classroom level without harsh
  // peaks if several filtered paths reinforce one another.
  ...
  bodyHighpass.connect(lowpass);
  lowpass.connect(lowGain);
  lowGain.connect(mix);

  source.connect(bandpass);
  bandpass.connect(textureGain);
  textureGain.connect(mix);

  source.connect(burbleFilter);
  burbleFilter.connect(burbleGain);
  burbleGain.connect(mix);
  ...
  soundEngine = {
    ctx, source, bodyHighpass, lowpass, lowGain,
    bandpass, textureGain, burbleFilter, burbleGain,
    mix, panner, master, compressor,
  };
  return true;
}
...
function updateFlowSonification(nowMs = performance.now(), force = false) {
  ...
  if (!shouldBeAudible) {
    setAudioTarget(soundEngine.master.gain, 0, 0.075);
    return;
  }
  ...
  // The developing current is intentionally a little exaggerated for younger
  // learners, but silence still corresponds to a genuinely still/cold system.
  ...
  const targetMaster =
    SOUND_TUNING.maxMasterGain * Math.pow(intensity, 0.64);

  // Keep energy in the range that ordinary tablet/laptop speakers reproduce.
  const bodyFrequency =
    470 + state.motion * 330 + state.warmth * 250 + state.flow * 150;
  const textureFrequency =
    760 + state.motion * 820 + state.flow * 520 + state.warmth * 160;

  // Quasi-irregular modulation gives the resonant path a gentle water/burble
  // character. Its LEVEL is still tied to actual vertical circulation.
  const slowWave = 0.5 + 0.5 * Math.sin(nowMs * 0.00225 + 0.7);
  const ripple = 0.5 + 0.5 * Math.sin(
    nowMs * 0.0051 + Math.sin(nowMs * 0.0011) * 1.5
  );
  const burblePulse = Math.pow(0.38 + slowWave * 0.25 + ripple * 0.37, 1.7);

  const burbleFrequency =
    320 + ripple * 420 + state.motion * 190 + state.warmth * 90;
  const burbleLevel =
    SOUND_TUNING.baseBurbleGain +
    state.vertical * 0.34 * burblePulse +
    state.flow * 0.065;

  const textureLevel =
    SOUND_TUNING.baseTextureGain +
    state.motion * 0.26 +
    state.flow * 0.18;

  const bodyLevel =
    SOUND_TUNING.bodyGain + state.flow * 0.16;

  setAudioTarget(soundEngine.master.gain, targetMaster, 0.105);
  setAudioTarget(soundEngine.lowpass.frequency, bodyFrequency, 0.13);
  setAudioTarget(soundEngine.lowGain.gain, bodyLevel, 0.15);

  setAudioTarget(soundEngine.bandpass.frequency, textureFrequency, 0.13);
  setAudioTarget(soundEngine.bandpass.Q, 1.1 + state.flow * 1.3, 0.16);
  setAudioTarget(soundEngine.textureGain.gain, textureLevel, 0.14);

  setAudioTarget(soundEngine.burbleFilter.frequency, burbleFrequency, 0.085);
  setAudioTarget(soundEngine.burbleFilter.Q, 3.8 + state.vertical * 3.0, 0.11);
  setAudioTarget(soundEngine.burbleGain.gain, burbleLevel, 0.075);

  if (soundEngine.panner) {
    setAudioTarget(soundEngine.panner.pan, state.pan, 0.17);
  }
}
...
function syncSoundUi() {
  ...
  soundToggleBtn.title = soundEnabled
    ? 'Sound on — exaggerated sonification follows the simulated flow'
    : 'Sound off — exaggerated flow sonification, not literal molecular sound';
}
...
async function setSoundEnabled(enabled) {
  ...
  if (soundEngine) {
    setAudioTarget(soundEngine.master.gain, 0, 0.055);
