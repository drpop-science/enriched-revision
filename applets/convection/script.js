// V15.6.4 classroom presentation refinement + aquarium sound effect
...
// V15.6.4 aquarium sound effect ---------------------------------------------
// IMPORTANT: The only concept/code adapted from the user's uploaded Claude
// prototype is its AUDIO treatment. No rendering, simulation, UI, particle,
// fluid, walkthrough or interaction code from that prototype is used here.
//
// Like the supplied prototype, the steady sound is intentionally atmosphere,
// not physics: a very quiet lowpassed brown-noise body. The separate 3.7 s
// heater gurgle is our clean locally bundled sample and is played only at
// meaningful interaction moments.
//
// Sound is OFF by default. A one-time toast makes the pedagogical status clear.
...
let lastGurgleWallTime = -Infinity;
const GURGLE_COOLDOWN_MS = 3300;
...
function buildBrownNoiseBuffer(ctx) {
  ...
  // Smooth loop seam, matching the quiet wandering-water body approach.
  const fade = Math.min(1200, Math.floor(len * 0.08));
  for (let i = 0; i < fade; i++) {
    const t = i / Math.max(1, fade);
...
async function ensureSoundEngine() {
  ...
  try {
    ...
    // Resume from the explicit Sound-button gesture for iPadOS/Safari.
    ...
    // Claude-style quiet low-band body: deliberately almost inaudible alone.
    const bedSource = ctx.createBufferSource();
    bedSource.buffer = buildBrownNoiseBuffer(ctx);
    bedSource.loop = true;
    ...
    // Load our short clean heater gurgle.
    const response = await fetch(HEATER_GURGLE_URL, { cache:'force-cache' });
    if (!response.ok) {
      throw new Error(`Could not load ${HEATER_GURGLE_URL} (${response.status})`);
    }
    const bytes = await response.arrayBuffer();
    const gurgleBuffer = await ctx.decodeAudioData(bytes);
...
function playHeaterGurgle({ force = false } = {}) {
  if (!soundEnabled || !soundEngine || !isRunning || !heaterOn) return false;

  const nowMs = performance.now();
  if (!force && nowMs - lastGurgleWallTime < GURGLE_COOLDOWN_MS) return false;
  lastGurgleWallTime = nowMs;

  const { ctx, gurgleBuffer, gurgleBus } = soundEngine;
  const source = ctx.createBufferSource();
  source.buffer = gurgleBuffer;

  // Small per-play variation keeps repeated heater movements from feeling
  // like the exact same button sound.
...
function updateAquariumAmbience(dtSeconds = 1/60) {
  ...
  if (!shouldBeAudible) {
    setAudioTarget(soundEngine.master.gain, 0, 0.12);
    setAudioTarget(soundEngine.bedGain.gain, 0, 0.22);
    return;
  }

  // Steady layer is atmosphere only, not mapped to the solver.
  // Slow random wandering mirrors the uploaded prototype's audio behaviour.
  const wander = 0.5 + 0.5 * Math.sin(performance.now() * 0.00031);
  setAudioTarget(soundEngine.master.gain, 0.72, 0.13);
  setAudioTarget(soundEngine.bedGain.gain, 0.008 + 0.005 * wander, 0.50);
  setAudioTarget(soundEngine.bedFilter.frequency, 285 + 65 * wander, 0.60);
}
...
function syncSoundUi() {
  ...
  if (!AudioContextClass) {
    ...
    soundToggleBtn.setAttribute(
      'aria-label',
      'Aquarium sound effect is not supported in this browser'
    );
...
async function setSoundEnabled(enabled) {
  ...
  if (enabled) {
    ...
    updateAquariumAmbience();
    if (heaterOn && isRunning) {
      playHeaterGurgle({ force:true });
    }
    return;
  }
  ...
  if (soundEngine) {
    ...
    setAudioTarget(soundEngine.bedGain.gain, 0, 0.12);
...
heaterOnBtn.addEventListener('click', () => {
  ...
  heaterOn = true;
  updateHeaterControls();
  if (wasOff) playHeaterGurgle({ force:true });
});
...
heaterPosition.addEventListener('pointerdown', () => {
  playHeaterGurgle();
});
heaterPosition.addEventListener('keydown', (event) => {
