// V14.1 resilient Three.js loader ------------------------------------------------
...
try {
  // V14.1: start all mirrors together. A slow/stalled provider can no longer
  // block the fallbacks because Promise.any resolves with the first success.
  const loaded = await Promise.any(THREE_SOURCES.map(importWithTimeout));
  THREE = loaded.module;
  threeSourceUsed = loaded.source;
} catch (error) {
  console.error('All Three.js sources failed:', error);
}
...
class OrbitControls {
  constructor(camera, domElement) {
    ...
    this.rotateSpeed = 0.82;
    this.zoomSpeed = 0.00125;
    this._thetaDelta = 0;
    this._phiDelta = 0;
    this._zoomScale = 1;
    ...
    this._onPointerMove = (e) => {
      ...
      if (this._pointers.size === 1) {
        ...
        if (this._lastPinch && dist > 2) {
          this._zoomScale *= this._lastPinch / dist;
        }
    ...
    this._onWheel = (e) => {
      if (!this.enabled) return;
      e.preventDefault();
      this._zoomScale *= Math.exp(e.deltaY * this.zoomSpeed);
    };
...
const backdropPanel = new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({ color: 0x414b53, transparent: true, opacity: 0.78, side: THREE.DoubleSide, depthWrite: false })
);
