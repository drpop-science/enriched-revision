...
const DENSITY_EQUALISATION_STRENGTH = 0.022;

// Normalised tweak factors (1.00 = V11 baseline)
const EFFECT_FACTORS = {
  backdropContrast: 1.18,
  wallStandOff: 0.84,
  continuityEqualisation: 1.18,
  hydrostaticPackingBias: 1.20,
  arrowThickness: 1.35,
  arrowOpacity: 1.30,
...
const backdropPanel = new THREE.Mesh(
  new THREE.PlaneGeometry(TANK.width + 1.6, TANK.height + 1.0),
  new THREE.MeshBasicMaterial({ color: 0xddeaf2, transparent: true, opacity: 0.42 * EFFECT_FACTORS.backdropContrast, side: THREE.DoubleSide, depthWrite: false })
);
backdropPanel.position.set(0, TANK.height * 0.55, -TANK.depth * 0.5 - 0.34);
backdropPanel.renderOrder = -1;
...
function makeParticles(count) {
  ...
  relaxParticlePacking(2);
  equalizeParticleDensity();
...
function resetSimulation() {
  ...
  explainText.textContent = 'The bulk current is now solved from heating, buoyancy and pressure rather than prescribed as a loop. A gentle continuity correction keeps the representative particles filling the liquid volume without obvious gaps.';
}
...
function applySoftWallForces(p, dt) {
  ...
  const xMargin = 0.42 * EFFECT_FACTORS.wallStandOff;
  const zMargin = 0.24 * EFFECT_FACTORS.wallStandOff;
  const bottomMargin = 0.14 * EFFECT_FACTORS.wallStandOff;
  const wallK = 7.2;
...
function equalizeParticleDensity() {
  ...
  const meanSpacing = Math.cbrt(volume / particles.length);
  const cellSize = Math.max(PARTICLE_RADIUS * 3.8, meanSpacing * 1.45);
  const nx = Math.max(4, Math.round(width / cellSize));
  const ny = Math.max(3, Math.round(height / cellSize));
  const nz = Math.max(2, Math.round(depth / cellSize));
  const size = nx * ny * nz;
  const counts = new Uint16Array(size);
  const tempSums = new Float32Array(size);
  const invHeight = 1 / height;

  const idx = (ix, iy, iz) => ix + nx * (iy + ny * iz);

  for (const p of particles) {
    const cx = THREE.MathUtils.clamp(Math.floor((p.x - BOUNDS.xMin) / cellSize), 0, nx - 1);
    const cy = THREE.MathUtils.clamp(Math.floor((p.y - BOUNDS.yMin) / cellSize), 0, ny - 1);
    const cz = THREE.MathUtils.clamp(Math.floor((p.z - BOUNDS.zMin) / cellSize), 0, nz - 1);
    p._dcx = cx; p._dcy = cy; p._dcz = cz;
    const id = idx(cx, cy, cz);
    counts[id] += 1;
    tempSums[id] += p.temp;
  }

  const meanCount = particles.length / size;

  for (const p of particles) {
    const cx = p._dcx, cy = p._dcy, cz = p._dcz;
    const id = idx(cx, cy, cz);
    const localCount = counts[id];
    if (!localCount) continue;

    const localTemp = tempSums[id] / localCount;
    const yNorm = (p.y - BOUNDS.yMin) * invHeight;
    const target = meanCount * (1 - 0.11 * localTemp + 0.07 * EFFECT_FACTORS.hydrostaticPackingBias * (1 - yNorm));
    const excess = localCount - target;
    if (excess < 0.55) continue;

    const left = counts[idx(Math.max(0, cx - 1), cy, cz)];
    const right = counts[idx(Math.min(nx - 1, cx + 1), cy, cz)];
    const down = counts[idx(cx, Math.max(0, cy - 1), cz)];
    const up = counts[idx(cx, Math.min(ny - 1, cy + 1), cz)];
    const back = counts[idx(cx, cy, Math.max(0, cz - 1))];
    const front = counts[idx(cx, cy, Math.min(nz - 1, cz + 1))];

    let gx = right - left;
    let gy = (up - down) * 0.78;
    let gz = (front - back) * 0.90;
    let gLen = Math.hypot(gx, gy, gz);
    if (gLen < 1e-5) {
      gx = Math.sin(p.seed * 1.1);
      gy = 0.35 * Math.cos(p.seed * 1.7);
      gz = 0.55 * Math.sin(p.seed * 0.9);
      gLen = Math.hypot(gx, gy, gz);
    }

    const strength = Math.min(0.032, (excess / Math.max(1, target)) * DENSITY_EQUALISATION_STRENGTH * EFFECT_FACTORS.continuityEqualisation);
    p.x -= (gx / gLen) * strength;
    p.y -= (gy / gLen) * strength;
    p.z -= (gz / gLen) * strength;
    confineParticle(p);
  }
}
...
function updateParticles(dt) {
  ...
  for (const p of particles) {
    ...
    // Very gentle hydrostatic settling bias: cooler particles settle slightly
    // more readily, helping the liquid fill the lower corners without
    // overriding the solved buoyant flow.
    p.vy -= 0.055 * EFFECT_FACTORS.hydrostaticPackingBias * (1 - 0.70 * p.temp) * dt;
...
for (let row = 0; row < arrowRows; row++) {
  for (let col = 0; col < arrowCols; col++) {
    const shaftMat = new THREE.MeshBasicMaterial({ color: 0x0c7fa0, transparent: true, opacity: 0.72, depthWrite: false });
    const coneMat = new THREE.MeshBasicMaterial({ color: 0x2ad1e3, transparent: true, opacity: 0.82, depthWrite: false });
    const glowMat = new THREE.MeshBasicMaterial({ color: 0x56f0ff, transparent: true, opacity: 0.22 * EFFECT_FACTORS.arrowGlow, depthWrite: false, blending: THREE.AdditiveBlending });
...
function updateFlowArrows() {
  ...
  for (const item of flowArrows) {
    ...
    const length = THREE.MathUtils.clamp(0.12 + speed * 0.40, 0.14, 0.58);
