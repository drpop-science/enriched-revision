...
camera.position.set(12.4, 6.5, 10.6);
...
const TANK = { width: 9.2, height: 4.35, depth: 4.6 };
const BOUNDS = {
...
function flowField(x, y, z, strength) {
  // V6: broadened closed circulation cell. The overall strength is raised by
...
function applyLiquidPressure(dt) {
  // A weak coarse-grained pressure/continuity correction. V5 could create
  ...
  for (const p of particles) {
    ...
    p.vz += (back - front) * pressureStrength * dt;
  }
}
...
function updateParticles(dt) {
  ...
  for (const p of particles) {
    ...
    const verticalHeat = Math.exp(-Math.pow((p.y - 0.32) / 0.46, 2));
...
const walkthroughSteps = [
  {
    ...
    view: () => ({
      position: new THREE.Vector3(heaterX + 2.8, 2.05, 3.25),
  ...
  },
  {
    ...
    view: () => ({
      position: new THREE.Vector3(heaterX + 3.5, 2.75, 4.3),
  ...
  },
  {
    ...
    view: () => ({
      position: new THREE.Vector3(heaterX + 4.1, 3.45, 4.8),
  ...
  },
  {
    ...
    view: () => ({
      position: new THREE.Vector3(12.4, 6.2, 10.2),
