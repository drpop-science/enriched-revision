// V15.5.3 classroom presentation refinement ---------------------------------------------------
...
const WALKTHROUGH_HEATER_X = 0.88;
...
function walkthroughFrontView() {
  return {
    ...
    position: new THREE.Vector3(0, 4.75, 12.10),
...
function enterWalkthroughStep(index) {
  ...
  // Step 2 uses controlled comparison volumes placed high in the tank so they
  // remain clear of the help banner and show a more obvious density contrast.
  if (walkthroughStep === 1 && step.overlay === 'both-volumes') {
    const halfW = sampleBox.width / 2;
    const halfH = sampleBox.height / 2;
    const topY = SURFACE_Y - halfH - 0.16;
    warmSamplePos.set(
      THREE.MathUtils.clamp(heaterX + 0.18, BOUNDS.xMin + halfW + 0.08, BOUNDS.xMax - halfW - 0.08),
      topY,
      0
    );
    coolSamplePos.set(BOUNDS.xMax - halfW - 0.22, topY, 0);
    warmCube.position.copy(warmSamplePos);
    coolCube.position.copy(coolSamplePos);
  } else {
    updateSamplePositions();
