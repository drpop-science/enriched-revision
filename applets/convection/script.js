// V15.6.2 classroom presentation refinement + underwater bubble-style sonification
// Copyright © 2026 Dr Pop · Dr Pop's Enriched Revision
// Educational content: CC BY-NC 4.0 · Source code: PolyForm Noncommercial 1.0.0 ---------------------------------------------------
...
const APPLET_ATTRIBUTION = Object.freeze({
  title: '3D Convection',
  version: 'V15.6.2',
  creator: 'Dr Pop',
  site: 'https://drpop-science.github.io/enriched-revision/',
  educationalContentLicense: 'CC BY-NC 4.0',
  educationalContentLicenseUrl: 'https://creativecommons.org/licenses/by-nc/4.0/',
  sourceCodeLicense: 'PolyForm Noncommercial 1.0.0',
  sourceCodeLicenseUrl: 'https://polyformproject.org/licenses/noncommercial/1.0.0/',
});
window.__DR_POP_APPLET__ = APPLET_ATTRIBUTION;
...
// V15.6.2 procedural underwater-style sonification --------------------------
// This is deliberately a SONIFICATION of the simulated convection current,
// not a claim that the model is literally producing gas bubbles.
//
// The sound direction is inspired by the user's underwater-bubble references:
// discrete rounded resonances, irregular clusters, and a muffled underwater
// bed rather than a continuous wind-like hiss. No audio is copied or sampled.
//
// The ACTUAL simulation drives:
//   - event rate        <- solved bulk-flow / vertical circulation
//   - event strength    <- circulation intensity
//   - spectral activity <- representative-particle movement / warmth
//   - stereo position   <- horizontal centre of simulated movement
//
// A small stochastic layer controls the timing/pitch of individual sonification
// events so repeated motion does not sound mechanical.
const AudioContextClass = window.AudioContext || window.webkitAudioContext;
let soundEnabled = false;
let soundEngine = null;
let lastSonificationMeasure = 0;
let sonificationSuspendTimer = null;
let nextBubbleEventAt = 0;
let bubbleBurstRemaining = 0;
let nextBurstEventAt = 0;
const SONIFICATION_MEASURE_MS = 75;

const SOUND_TUNING = Object.freeze({
  maxMasterGain: 0.26,
  stereoWidth: 0.46,

  // Continuous underwater bed is deliberately quiet so it never reads as wind.
  bedGain: 0.115,
  bedTextureGain: 0.048,

  // Discrete resonant events carry most of the audible identity.
  bubbleBusGain: 0.92,
  minEventRate: 0.15,
  maxEventRate: 10.5,
  maxBurstSize: 3,
...
function makeFlowNoiseBuffer(ctx) {
  const duration = 5.0;
  const length = Math.max(1, Math.floor(ctx.sampleRate * duration));
  const buffer = ctx.createBuffer(1, length, ctx.sampleRate);
  const data = buffer.getChannelData(0);

  // Pink-ish noise gives the faint continuous water bed more body and much
  // less top-end hiss than white noise.
  ...
  for (let i = 0; i < length; i++) {
    ...
    b6 = white*0.115926;
    data[i] = THREE.MathUtils.clamp(pink * 0.105, -1, 1);
  }

  // Crossfade the loop boundary.
  const fadeLength = Math.min(4096, Math.floor(length * 0.045));
...
function makeTransientNoiseBuffer(ctx) {
...
function ensureSoundEngine() {
  ...
  bedSource.buffer = makeFlowNoiseBuffer(ctx);
  bedSource.loop = true;

  const bedHighpass = ctx.createBiquadFilter();
  bedHighpass.type = 'highpass';
  bedHighpass.frequency.value = 115;
  bedHighpass.Q.value = 0.5;

  const bedLowpass = ctx.createBiquadFilter();
  bedLowpass.type = 'lowpass';
  bedLowpass.frequency.value = 520;
  bedLowpass.Q.value = 0.72;

  const bedGain = ctx.createGain();
  bedGain.gain.value = SOUND_TUNING.bedGain;

  const bedTexture = ctx.createBiquadFilter();
  bedTexture.type = 'bandpass';
  bedTexture.frequency.value = 940;
  bedTexture.Q.value = 1.5;

  const bedTextureGain = ctx.createGain();
  bedTextureGain.gain.value = SOUND_TUNING.bedTextureGain;

  // ------------------ discrete bubble-style events ----------
  const bubbleBus = ctx.createGain();
  bubbleBus.gain.value = SOUND_TUNING.bubbleBusGain;

  const bubbleLowpass = ctx.createBiquadFilter();
  bubbleLowpass.type = 'lowpass';
  bubbleLowpass.frequency.value = 2350;
  bubbleLowpass.Q.value = 0.55;
  ...
  bedSource.connect(bedHighpass);
  bedHighpass.connect(bedLowpass);
  bedLowpass.connect(bedGain);
  bedGain.connect(mix);

  bedSource.connect(bedTexture);
  bedTexture.connect(bedTextureGain);
  bedTextureGain.connect(mix);

  bubbleBus.connect(bubbleLowpass);
  bubbleLowpass.connect(mix);
  ...
  soundEngine = {
    ...
    master,
    compressor,
    transientNoiseBuffer: makeTransientNoiseBuffer(ctx),
  };

  return true;
}

function setAudioTarget(param, value, seconds = 0.10) {
  if (!soundEngine || !param) return;
  param.setTargetAtTime(value, soundEngine.ctx.currentTime, seconds);
}

function measureSonificationState() {
  if (!particles.length) {
    return { flow:0, vertical:0, motion:0, warmth:0, pan:0 };
  }

  const targetSamples = Math.min(96, particles.length);
  const stride = Math.max(1, Math.floor(particles.length / targetSamples));
  let n=0, totalBulk=0, totalVertical=0, totalMotion=0;
  let totalWarmth=0, xEnergy=0, energyTotal=0;

  for (let i = 0; i < particles.length; i += stride) {
    const p = particles[i];
    const bulk = fluid.velocityAt(p.x, p.y);
    const vx = p.vx + bulk.x * FLOW_PARTICLE_SCALE;
    const vy = p.vy + bulk.y * FLOW_PARTICLE_SCALE;
    const particleMotion = Math.hypot(vx, vy, p.vz);
    const bulkSpeed = Math.hypot(bulk.x, bulk.y);
    const movementEnergy = bulkSpeed * 0.84 + particleMotion * 0.16;

    totalBulk += bulkSpeed;
    totalVertical += Math.abs(bulk.y);
    totalMotion += particleMotion;
    totalWarmth += p.temp;
    xEnergy += p.x * movementEnergy;
    energyTotal += movementEnergy;
    n++;
  }

  n = Math.max(1, n);
  const avgBulk = totalBulk / n;
  const avgVertical = totalVertical / n;
  const avgMotion = totalMotion / n;
  const avgWarmth = totalWarmth / n;

  const flow = THREE.MathUtils.clamp(avgBulk / 0.68, 0, 1);
  const vertical = THREE.MathUtils.clamp(avgVertical / 0.44, 0, 1);
  const motion = THREE.MathUtils.clamp((avgMotion - 0.022) / 0.68, 0, 1);
  const warmth = THREE.MathUtils.clamp(avgWarmth / 0.40, 0, 1);

  const energyX = energyTotal > 1e-5 ? xEnergy / energyTotal : heaterX;
  const pan = THREE.MathUtils.clamp(
    energyX / Math.max(0.1, TANK.width * 0.5),
    -1,
    1
  ) * SOUND_TUNING.stereoWidth;

  return { flow, vertical, motion, warmth, pan };
}

function scheduleBubbleResonance(state, when = soundEngine?.ctx.currentTime ?? 0) {
  if (!soundEngine) return;

  const { ctx } = soundEngine;
  const sizeBias = Math.pow(Math.random(), 0.72); // more mid/small "bubbles"
  const duration = 0.075 + (1 - sizeBias) * 0.19 + Math.random() * 0.055;

  // Rounded underwater resonances: larger events are lower and longer.
  const baseFreq =
    360 +
    sizeBias * 970 +
    state.motion * 150 +
  ...
  const startFreq = baseFreq * (0.80 + Math.random() * 0.10);
  const endFreq = baseFreq * (1.08 + Math.random() * 0.24);

  const strength =
    0.020 +
    state.vertical * 0.048 +
    state.flow * 0.032 +
    Math.random() * 0.018;

  // Primary damped resonant tone.
  const osc = ctx.createOscillator();
  osc.type = 'sine';

  const resonantGain = ctx.createGain();
  resonantGain.gain.setValueAtTime(0.0001, when);
  resonantGain.gain.exponentialRampToValueAtTime(
    Math.max(0.0002, strength),
    when + 0.006
  );
  resonantGain.gain.exponentialRampToValueAtTime(
    0.0001,
  ...
  osc.frequency.setValueAtTime(Math.max(80, startFreq), when);
  osc.frequency.exponentialRampToValueAtTime(
    Math.max(90, endFreq),
    when + duration * 0.72
  );

  // A quieter second partial prevents the event sounding like a pure UI beep.
  const partial = ctx.createOscillator();
  partial.type = 'sine';
  partial.frequency.setValueAtTime(startFreq * 1.46, when);
  ...
  partialGain.gain.exponentialRampToValueAtTime(
    Math.max(0.00015, strength * 0.19),
    when + 0.004
  );
  partialGain.gain.exponentialRampToValueAtTime(
    0.0001,
  ...
  // Tiny filtered transient gives each resonance a soft "blup" onset.
  const transient = ctx.createBufferSource();
  transient.buffer = soundEngine.transientNoiseBuffer;

  const transientFilter = ctx.createBiquadFilter();
  transientFilter.type = 'bandpass';
  transientFilter.frequency.value = baseFreq * 1.18;
  transientFilter.Q.value = 2.4 + Math.random() * 2.2;

  const transientGain = ctx.createGain();
  transientGain.gain.setValueAtTime(0.0001, when);
  transientGain.gain.exponentialRampToValueAtTime(
    Math.max(0.00015, strength * 0.16),
    when + 0.0025
  );
  transientGain.gain.exponentialRampToValueAtTime(
    0.0001,
  ...
  // Slight per-event stereo scatter around the flow centre.
  let eventOutput = soundEngine.bubbleBus;
  let eventPanner = null;
  if (typeof ctx.createStereoPanner === 'function') {
    eventPanner = ctx.createStereoPanner();
    eventPanner.pan.value = THREE.MathUtils.clamp(
      state.pan + (Math.random() - 0.5) * 0.22,
      -0.72,
      0.72
    );
    eventPanner.connect(soundEngine.bubbleBus);
    eventOutput = eventPanner;
  }

  osc.connect(resonantGain);
  resonantGain.connect(eventOutput);

  partial.connect(partialGain);
  partialGain.connect(eventOutput);

  transient.connect(transientFilter);
  transientFilter.connect(transientGain);
  transientGain.connect(eventOutput);
  ...
  osc.addEventListener('ended', () => {
    ...
    try { resonantGain.disconnect(); } catch {}
    try { partial.disconnect(); } catch {}
    try { partialGain.disconnect(); } catch {}
    try { transient.disconnect(); } catch {}
    try { transientFilter.disconnect(); } catch {}
    try { transientGain.disconnect(); } catch {}
    try { eventPanner?.disconnect(); } catch {}
  }, { once:true });
}

function scheduleSonificationEvents(state, nowMs) {
  if (!soundEngine) return;

  const circulation = THREE.MathUtils.clamp(
    state.flow * 0.58 +
    state.vertical * 0.31 +
    state.warmth * 0.11,
    0,
  ...
  // A genuinely still/cold system remains virtually silent.
  if (circulation < 0.025) {
    nextBubbleEventAt = nowMs + 280;
    bubbleBurstRemaining = 0;
    return;
  }

  const rate =
    SOUND_TUNING.minEventRate +
    Math.pow(circulation, 0.72) *
      (SOUND_TUNING.maxEventRate - SOUND_TUNING.minEventRate);

  if (!nextBubbleEventAt) nextBubbleEventAt = nowMs + 120;

  if (bubbleBurstRemaining > 0 && nowMs >= nextBurstEventAt) {
    scheduleBubbleResonance(
      state,
      soundEngine.ctx.currentTime + 0.008 + Math.random() * 0.018
    );
    bubbleBurstRemaining--;
    nextBurstEventAt = nowMs + 32 + Math.random() * 78;
  }

  if (nowMs < nextBubbleEventAt) return;
  ...
  // Strong vertical circulation occasionally gives a short cluster rather than
  // an evenly spaced stream of identical events.
  const burstChance = 0.10 + state.vertical * 0.34 + state.flow * 0.10;
  if (Math.random() < burstChance) {
    bubbleBurstRemaining = Math.min(
      SOUND_TUNING.maxBurstSize - 1,
      1 + (Math.random() < state.vertical * 0.62 ? 1 : 0)
    );
    nextBurstEventAt = nowMs + 38 + Math.random() * 72;
  }

  // Exponential-ish timing avoids a metronomic pattern.
  const meanInterval = 1000 / Math.max(0.15, rate);
  const jitter = 0.48 + (-Math.log(Math.max(0.06, Math.random()))) * 0.58;
  nextBubbleEventAt = nowMs + THREE.MathUtils.clamp(
    meanInterval * jitter,
    55,
    1200
  );
...
function updateFlowSonification(nowMs = performance.now(), force = false) {
  if (!soundEngine) return;
  ...
  const shouldBeAudible =
    ...
    isRunning &&
    !document.hidden &&
    examAnswerCard.hidden;

  if (!shouldBeAudible) {
    setAudioTarget(soundEngine.master.gain, 0, 0.065);
    bubbleBurstRemaining = 0;
    return;
  }

  if (!force && nowMs - lastSonificationMeasure < SONIFICATION_MEASURE_MS) return;
  lastSonificationMeasure = nowMs;

  const state = measureSonificationState();
  ...
  // Much more audible than V15.6/V15.6.1, but discrete resonances rather than
  // a loud continuous noise bed carry most of the perceived volume.
  const targetMaster =
    SOUND_TUNING.maxMasterGain * Math.pow(intensity, 0.55);

  const bedLevel =
    SOUND_TUNING.bedGain * (0.28 + intensity * 0.72);
  const textureLevel =
    SOUND_TUNING.bedTextureGain * (0.22 + state.motion * 0.78);

  const bedCutoff =
    390 + state.flow * 250 + state.motion * 170 + state.warmth * 120;
  const textureFreq =
    760 + state.motion * 590 + state.flow * 280;

  setAudioTarget(soundEngine.master.gain, targetMaster, 0.085);
  setAudioTarget(soundEngine.bedGain.gain, bedLevel, 0.15);
  setAudioTarget(soundEngine.bedLowpass.frequency, bedCutoff, 0.16);
  setAudioTarget(soundEngine.bedTexture.frequency, textureFreq, 0.16);
  setAudioTarget(soundEngine.bedTextureGain.gain, textureLevel, 0.14);

  if (soundEngine.panner) {
    setAudioTarget(soundEngine.panner.pan, state.pan * 0.46, 0.17);
  }

  scheduleSonificationEvents(state, nowMs);
}

function syncSoundUi() {
  setSceneToolState(soundToggleBtn, soundEnabled);

  if (!AudioContextClass) {
    soundToggleBtn.disabled = true;
    soundToggleBtn.setAttribute(
      'aria-label',
      'Flow sonification is not supported in this browser'
    );
    soundToggleBtn.title = 'Flow sonification is not supported in this browser';
    return;
  }
  ...
  soundToggleBtn.setAttribute(
    'aria-label',
    soundEnabled ? 'Turn off flow sonification' : 'Turn on flow sonification'
  );
  soundToggleBtn.title = soundEnabled
    ? 'Sound on — underwater-style sonification follows the simulated current'
    : 'Sound off — underwater-style flow sonification, not literal molecular sound';
}

async function setSoundEnabled(enabled) {
  clearTimeout(sonificationSuspendTimer);
  ...
  if (enabled) {
    ...
    try {
      if (soundEngine.ctx.state !== 'running') {
        await soundEngine.ctx.resume();
      }
    } catch (error) {
      console.warn('Could not resume flow sonification:', error);
    }

    nextBubbleEventAt = performance.now() + 70;
    bubbleBurstRemaining = 0;
    updateFlowSonification(performance.now(), true);
    return;
  }
  ...
  if (soundEngine) {
    setAudioTarget(soundEngine.master.gain, 0, 0.05);
    sonificationSuspendTimer = setTimeout(() => {
      if (!soundEnabled && soundEngine?.ctx.state === 'running') {
...
function projectHeaterRailToScreen() {
  ...
  return {
    dx: heaterProjectionB.x - heaterProjectionA.x,
    dy: heaterProjectionB.y - heaterProjectionA.y,
  };
}

function getHeaterViewSign() {
  // Camera local +X is the direction of screen-right in world space.
  // Dotting it with the heater's world +X rail tells us whether increasing
  // world-x currently appears to move right (+) or left (-) on screen.
  heaterCameraRight.set(1, 0, 0).applyQuaternion(camera.quaternion).normalize();
  const projection = heaterCameraRight.x;

  // Near a side-on view the heater rail is highly foreshortened and "left/right"
  // becomes visually ambiguous. Retain the last stable mapping rather than
  // allowing the slider to flip back and forth.
  if (Math.abs(projection) < 0.075) return lastHeaterViewSign || 1;
  return projection >= 0 ? 1 : -1;
}

function heaterWorldToVisualX(worldX) {
  return worldX * getHeaterViewSign();
}

function heaterVisualToWorldX(visualX) {
  return visualX * getHeaterViewSign();
}

function syncHeaterPositionUi() {
  const visualX = heaterWorldToVisualX(heaterX);
  heaterPosition.value = visualX.toFixed(2);
  heaterPositionValue.textContent = formatHeaterPosition(visualX);
}

function setHeaterX(x) {
...
function resetSimulation() {
  ...
  explainText.textContent = 'Grab the heater in the diagram and drag it left or right, or use the position slider below.';
}

const FLUID_STATE_KEYS = ['u', 'v', 'T', 'u0', 'v0', 'T0', 'p', 'div', 'curl'];

function captureSimulationSnapshot() {
  const fluidState = {};
  for (const key of FLUID_STATE_KEYS) {
    fluidState[key] = fluid[key].slice();
  }
  ...
  return {
    ...
    particles: particles.map(p => ({
      ...
      temp: p.temp,
      seed: p.seed,
      packJitter: p.packJitter,
      _pcx: p._pcx, _pcy: p._pcy, _pcz: p._pcz,
    })),
  };
}

function resetFlowForTutorial() {
  // Start the storyboard from a genuinely cold, stationary bulk-flow state.
  // Keep the existing representative-particle positions so entering Tutorial
  // feels immediate rather than visibly reseeding the entire liquid.
  elapsed = 0;
  fluid = new Fluid2D();
  gaussianSpare = null;
  trailAccumulator = 0;
  ...
  for (const p of particles) {
    p.temp = 0;
    p.vx = gaussian() * PARTICLE_DYNAMICS.jitterCold * 0.18;
    p.vy = gaussian() * PARTICLE_DYNAMICS.jitterCold * 0.18;
    p.vz = gaussian() * PARTICLE_DYNAMICS.jitterCold * 0.18 * DEPTH_MOTION_SCALE;
  }
...
function restoreSimulationSnapshot(snapshot) {
  ...
  fluid = new Fluid2D();
  for (const key of FLUID_STATE_KEYS) {
    if (!snapshot.fluid?.[key]) return false;
    fluid[key].set(snapshot.fluid[key]);
  }

  if (particles.length !== snapshot.particles.length) {
    seedParticles(snapshot.particles.length);
  }

  for (let i = 0; i < snapshot.particles.length; i++) {
    Object.assign(particles[i], snapshot.particles[i]);
  }

  elapsed = snapshot.elapsed;
  isRunning = snapshot.isRunning;
  heaterOn = snapshot.heaterOn;
  setHeaterX(snapshot.heaterX);
  trailAccumulator = 0;
  clearTraceObjects();
...
function setTutorialControlsLocked(locked) {
  tutorialLockControls.forEach(control => {
    if (control) control.disabled = locked;
  });

  // Keep Tutorial itself available so pressing it again can close the guide.
  // Keep presentation/fullscreen available because it does not alter the model.
  walkthroughBtn.disabled = false;
  fullscreenBtn.disabled = false;
  controlPanel?.classList.toggle('tutorial-locked', locked);
}
...
const WALKTHROUGH_HEATER_X = -1.38;
let heaterTween = null;
let lastHeaterViewSign = 1;
...
function updateNarration() {
  ...
  if (explainText.textContent !== 'Grab the heater in the diagram and drag it left or right, or use the position slider below.') {
    explainText.textContent = 'Grab the heater in the diagram and drag it left or right, or use the position slider below.';
  }
}
...
function walkthroughFrontView() {
  const canUseCloserTeachingView =
    isPresentationFullscreen() || shouldDockTutorialOverSettings();

  return {
    position: canUseCloserTeachingView
      ? new THREE.Vector3(0, 4.82, 11.65)
      : new THREE.Vector3(0, 4.95, 12.35),
    target: new THREE.Vector3(0, 1.48, 0),
  };
}

function shouldDockTutorialOverSettings() {
  return !isPresentationFullscreen() &&
    window.matchMedia('(orientation: landscape) and (min-width: 901px)').matches;
}

function placeWalkthroughBanner() {
  if (!walkthroughBanner) return;

  if (walkthroughActive && shouldDockTutorialOverSettings()) {
    if (walkthroughBanner.parentElement !== controlPanel) {
      controlPanel.appendChild(walkthroughBanner);
    }
  } else {
    if (walkthroughBanner.parentElement !== sceneHost) {
      sceneHost.appendChild(walkthroughBanner);
    }
  }
}

let tutorialAttentionTimer = null;

function syncTutorialPageFocus() {
  document.body.classList.toggle(
    'tutorial-page-focus',
    walkthroughActive && !isPresentationFullscreen()
  );
}

function pulseTutorialCard() {
  clearTimeout(tutorialAttentionTimer);
  walkthroughBanner.classList.remove('attention-pulse');
  void walkthroughBanner.offsetWidth;
  walkthroughBanner.classList.add('attention-pulse');
  tutorialAttentionTimer = setTimeout(() => {
    walkthroughBanner.classList.remove('attention-pulse');
  }, 2350);
}
...
function startHeaterTween(targetX, duration = 760) {
  if (prefersReducedMotion) {
    setHeaterX(targetX);
    heaterTween = null;
    return;
  ...
  heaterTween = {
    ...
    duration,
    fromX: heaterX,
    toX: THREE.MathUtils.clamp(targetX, -HEATER_X_LIMIT, HEATER_X_LIMIT),
  };
}

function updateHeaterTween(now) {
  if (!heaterTween) return;
  const raw = THREE.MathUtils.clamp((now - heaterTween.start) / heaterTween.duration, 0, 1);
  const eased = raw < 0.5 ? 4 * raw * raw * raw : 1 - Math.pow(-2 * raw + 2, 3) / 2;
  setHeaterX(THREE.MathUtils.lerp(heaterTween.fromX, heaterTween.toX, eased));
  if (walkthroughActive && walkthroughStep === 1) positionTutorialComparisonVolumes();
  if (raw >= 1) heaterTween = null;
}
...
function positionTutorialComparisonVolumes() {
  const halfW = sampleBox.width / 2;
  const halfH = sampleBox.height / 2;
  const topY = SURFACE_Y - halfH - 0.14;

  // Red: high in the warm plume, close to the water surface.
  warmSamplePos.set(
    THREE.MathUtils.clamp(heaterX + 0.12, BOUNDS.xMin + halfW + 0.08, BOUNDS.xMax - halfW - 0.08),
    topY,
    0
  );

  // Blue: high at the top-right corner of the water, well separated from the plume.
  coolSamplePos.set(
    BOUNDS.xMax - halfW - 0.18,
    topY,
    0
  );
...
function enterWalkthroughStep(index) {
  ...
  // Step 2 keeps the two comparison volumes high and well separated from
  // both the tutorial banner and one another.
  if (walkthroughStep === 1 && step.overlay === 'both-volumes') {
    positionTutorialComparisonVolumes();
  } else {
    updateSamplePositions();
  }
...
function startWalkthrough() {
  ...
  walkthroughSavedState = {
    ...
    heaterX,
    simulation: captureSimulationSnapshot(),
  };

  walkthroughActive = true;
  walkthroughBtn.disabled = false;
  walkthroughStopBtn.disabled = false;
  setTutorialControlsLocked(true);
  syncSceneToolbar();
  controls.enabled = false;
  placeWalkthroughBanner();
  syncTutorialPageFocus();
  pulseTutorialCard();

  // Clean storyboard start: move the heater first, then clear all accumulated
  // temperature and bulk flow before allowing the tutorial to run.
  heaterTween = null;
  heaterOn = false;
  setHeaterX(WALKTHROUGH_HEATER_X);
  resetFlowForTutorial();
  heaterOn = true;
  updateHeaterControls();
...
function finishWalkthrough(restoreView = false) {
  ...
  setTutorialControlsLocked(false);
  syncSceneToolbar();
  ...
  syncTutorialPageFocus();
  if (walkthroughBanner.parentElement !== sceneHost) sceneHost.appendChild(walkthroughBanner);

  if (walkthroughSavedState) {
    const restoredSimulation = restoreSimulationSnapshot(walkthroughSavedState.simulation);
    ...
    // Snapshot restoration already puts the heater back exactly where it was.
    // Keep the old fallback only if a snapshot could not be restored.
    if (!restoredSimulation) {
      startHeaterTween(walkthroughSavedState.heaterX, restoreView ? 720 : 0);
    }
...
soundToggleBtn.addEventListener('click', () => { void setSoundEnabled(!soundEnabled); });

document.addEventListener('visibilitychange', () => {
  // requestAnimationFrame may be throttled while hidden, so silence the audio
  // immediately rather than waiting for the next visual frame.
  if (document.hidden && soundEngine) {
    setAudioTarget(soundEngine.master.gain, 0, 0.04);
  } else if (!document.hidden && soundEnabled && soundEngine?.ctx.state === 'running') {
    updateFlowSonification(performance.now(), true);
  }
});


let examAnswerSavedRunning = null;
let examAnswerSavedControlsEnabled = null;

function setExamAnswerOpen(open) {
  const currentlyOpen = !examAnswerCard.hidden;
  if (open === currentlyOpen) return;

  if (open) {
    examAnswerSavedRunning = isRunning;
    examAnswerSavedControlsEnabled = controls.enabled;

    isRunning = false;
    controls.enabled = false;
    updateStatus();

    examAnswerCard.hidden = false;
    sceneHost.classList.add('exam-answer-open');
  } else {
    examAnswerCard.hidden = true;
    sceneHost.classList.remove('exam-answer-open');

    if (examAnswerSavedRunning !== null) {
      isRunning = examAnswerSavedRunning;
      updateStatus();
    }

    controls.enabled =
      examAnswerSavedControlsEnabled !== null
        ? examAnswerSavedControlsEnabled
        : !walkthroughActive;

    examAnswerSavedRunning = null;
    examAnswerSavedControlsEnabled = null;
  }
...
function applyPresentationViewportSize() {
...
function clearPresentationViewportSize() {
  for (const property of ['position', 'left', 'top', 'right', 'bottom', 'width', 'height']) {
    scenePanel.style[property] = '';
  }
}
...
function syncFullscreenUi() {
  ...
  if (active) applyPresentationViewportSize();
  placeWalkthroughBanner();
  syncTutorialPageFocus();
  requestAnimationFrame(() => resizeRenderer());
}
...
function exitPresentationFullscreen() {
  document.documentElement.classList.remove('presentation-root');
  document.body.classList.remove('presentation-mode');
  scenePanel.classList.remove('presentation-fullscreen');
  clearPresentationViewportSize();
  syncFullscreenUi();
}
...
function refreshPresentationLayout() {
  if (isPresentationFullscreen()) applyPresentationViewportSize();
  placeWalkthroughBanner();
  syncTutorialPageFocus();
}

window.addEventListener('resize', refreshPresentationLayout);
window.addEventListener('orientationchange', () => {
  setTimeout(refreshPresentationLayout, 80);
  setTimeout(refreshPresentationLayout, 320);
});
window.visualViewport?.addEventListener('resize', refreshPresentationLayout);
window.visualViewport?.addEventListener('scroll', refreshPresentationLayout);

const HEATER_NUDGE = 0.25;
heaterLeftBtn.addEventListener('click', () => {
  const sign = getHeaterViewSign();
  setHeaterX(heaterX - sign * HEATER_NUDGE);
});
...
function beginViewRelativeHeaterDrag(event) {
  ...
  let { dx: railDx, dy: railDy } = projectHeaterRailToScreen();
...
heaterPosition.addEventListener('input', () => {
  setHeaterX(heaterVisualToWorldX(Number(heaterPosition.value)));
});
...
function animate(now) {
  ...
  updateHeaterTween(now);
  controls.update();

  const heaterViewSign = getHeaterViewSign();
  if (heaterViewSign !== lastHeaterViewSign) {
    lastHeaterViewSign = heaterViewSign;
    syncHeaterPositionUi();
  }

  updateFlowArrows();
  updateFlowSonification(now);
...
async function initialiseApplet() {
  ...
  updateSamplePositions();
  lastHeaterViewSign = getHeaterViewSign();
  setHeaterX(heaterX);
  updateStatus();
  updateHeaterControls();
  syncDensityUi();
