// V15.6 classroom presentation refinement + procedural flow sonification
...
// V15.6 procedural flow sonification -----------------------------------------
// This is a SONIFICATION of the simulation state, not literal molecular sound.
// Audio is generated locally and continuously driven by the existing solved
// bulk-flow field plus representative-particle movement.
const AudioContextClass = window.AudioContext || window.webkitAudioContext;
let soundEnabled = false;
let soundEngine = null;
let lastSonificationMeasure = 0;
let sonificationSuspendTimer = null;
const SONIFICATION_MEASURE_MS = 90;
...
function makeFlowNoiseBuffer(ctx) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    slow = slow * 0.986 + white * 0.014;
    data[i] = THREE.MathUtils.clamp(white * 0.38 + slow * 2.15, -1, 1) * 0.62;
  }

  // Crossfade the tail towards the opening samples so the looping procedural
  // buffer does not introduce a synthetic click every few seconds.
  const fadeLength = Math.min(2048, Math.floor(length * 0.04));
...
function ensureSoundEngine() {
  ...
  source.connect(lowpass); lowpass.connect(lowGain); lowGain.connect(mix);
  source.connect(bandpass); bandpass.connect(textureGain); textureGain.connect(mix);
  if (panner) { mix.connect(panner); panner.connect(master); }
  else { mix.connect(master); }
  master.connect(ctx.destination);
  source.start();

  soundEngine = { ctx, source, lowpass, bandpass, lowGain, textureGain, mix, panner, master };
  return true;
}
...
function measureSonificationState() {
  if (!particles.length) return { flow:0, motion:0, warmth:0, pan:0 };
  const targetSamples = Math.min(84, particles.length);
  const stride = Math.max(1, Math.floor(particles.length / targetSamples));
  let n=0, totalBulk=0, totalMotion=0, totalWarmth=0, xEnergy=0, energyTotal=0;
  ...
  for (let i = 0; i < particles.length; i += stride) {
    ...
    const movementEnergy = bulkSpeed * 0.82 + particleMotion * 0.18;
    totalBulk += bulkSpeed;
  ...
  const flow = THREE.MathUtils.clamp(avgBulk / 0.72, 0, 1);
  const motion = THREE.MathUtils.clamp((avgMotion - 0.025) / 0.72, 0, 1);
  const warmth = THREE.MathUtils.clamp(avgWarmth / 0.42, 0, 1);
  const energyX = energyTotal > 1e-5 ? xEnergy / energyTotal : heaterX;
  const pan = THREE.MathUtils.clamp(energyX / Math.max(0.1, TANK.width * 0.5), -1, 1) * 0.34;
  return { flow, motion, warmth, pan };
}

function updateFlowSonification(nowMs = performance.now(), force = false) {
  if (!soundEngine) return;
  const shouldBeAudible = soundEnabled && isRunning && !document.hidden && examAnswerCard.hidden;
  if (!shouldBeAudible) {
    setAudioTarget(soundEngine.master.gain, 0, 0.08);
    return;
  }
  ...
  const intensity = THREE.MathUtils.clamp(state.flow * 0.82 + state.warmth * 0.18, 0, 1);
  const targetMaster = 0.044 * Math.pow(intensity, 0.78);
  const lowFrequency = 115 + state.motion * 260 + state.warmth * 85;
  const textureFrequency = 360 + state.motion * 720 + state.flow * 250;
  const textureLevel = 0.12 + state.motion * 0.30 + state.flow * 0.12;

  setAudioTarget(soundEngine.master.gain, targetMaster, 0.12);
  setAudioTarget(soundEngine.lowpass.frequency, lowFrequency, 0.14);
  setAudioTarget(soundEngine.bandpass.frequency, textureFrequency, 0.14);
  setAudioTarget(soundEngine.bandpass.Q, 0.72 + state.flow * 1.35, 0.16);
  setAudioTarget(soundEngine.textureGain.gain, textureLevel, 0.14);
  if (soundEngine.panner) setAudioTarget(soundEngine.panner.pan, state.pan, 0.18);
}
...
function syncSoundUi() {
  ...
  if (!AudioContextClass) {
    ...
    soundToggleBtn.setAttribute('aria-label', 'Flow sonification is not supported in this browser');
    soundToggleBtn.title = 'Flow sonification is not supported in this browser';
    return;
  }
  soundToggleBtn.disabled = false;
  soundToggleBtn.setAttribute('aria-label', soundEnabled ? 'Turn off flow sonification' : 'Turn on flow sonification');
  soundToggleBtn.title = soundEnabled
    ? 'Sound on — sonification follows the simulated particle and bulk-flow movement'
    : 'Sound off — sonification of simulated particle and bulk-flow movement, not literal molecular sound';
}

async function setSoundEnabled(enabled) {
  clearTimeout(sonificationSuspendTimer);
  if (enabled) {
    if (!ensureSoundEngine()) { soundEnabled = false; syncSoundUi(); return; }
    soundEnabled = true;
    syncSoundUi();
    try { if (soundEngine.ctx.state !== 'running') await soundEngine.ctx.resume(); }
    catch (error) { console.warn('Could not resume flow sonification:', error); }
  ...
  if (soundEngine) {
    ...
    sonificationSuspendTimer = setTimeout(() => {
      if (!soundEnabled && soundEngine?.ctx.state === 'running') soundEngine.ctx.suspend().catch(() => {});
    }, 240);
  }
}
