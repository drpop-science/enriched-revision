...
function equalizeParticleDensity() {
  ...
  for (const p of particles) {
    ...
    const target = meanCount * (1 - 0.11 * localTemp + 0.07 * (1 - yNorm));
    ...
    const strength = Math.min(0.028, (excess / Math.max(1, target)) * DENSITY_EQUALISATION_STRENGTH);
...
for (let row = 0; row < arrowRows; row++) {
  for (let col = 0; col < arrowCols; col++) {
    ...
    arrow.line.material.color.setHex(0x126b85);
    arrow.line.material.transparent = true;
    arrow.line.material.opacity = 0.72;
    arrow.line.material.depthWrite = false;
    arrow.cone.material.color.setHex(0x1e9bb3);
...
function updateFlowArrows() {
  ...
  for (const item of flowArrows) {
    ...
    const length = THREE.MathUtils.clamp(0.10 + speed * 0.40, 0.12, 0.56);
    item.arrow.position.set(x, y, 0);
    item.arrow.setDirection(dir);
    item.arrow.setLength(length, Math.min(0.135, length * 0.34), Math.min(0.080, length * 0.22));
    const alpha = THREE.MathUtils.clamp(0.38 + speed / 1.20, 0.40, 0.96);
    item.arrow.line.material.opacity = Math.max(0.44, alpha * 0.92);
...
function pickTracedParticles({ preferHeater = false } = {}) {
  ...
  for (const index of chosen) {
    ...
    const line = new THREE.Line(
      geometry,
      new THREE.LineBasicMaterial({ color: 0xf0b33c, transparent: true, opacity: 0.92, depthWrite: false })
    );
    const marker = new THREE.Mesh(
      new THREE.TorusGeometry(PARTICLE_RADIUS * 2.35, PARTICLE_RADIUS * 0.28, 10, 28),
      new THREE.MeshBasicMaterial({ color: 0xffc247, transparent: true, opacity: 0.98, depthWrite: false })
    );
