...
function updateFlowArrows() {
  ...
  for (const item of flowArrows) {
    ...
    item.shaftMat.opacity = Math.min(0.98, alpha * 0.96);
    item.coneMat.opacity = Math.min(1.0, alpha);
    item.glowMat.opacity = Math.min(0.50, 0.19 + speed * 0.09) * EFFECT_FACTORS.arrowGlow;
  }
}
