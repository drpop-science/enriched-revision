...
for (const source of THREE_SOURCES) {
  try {
    ...
    threeSourceUsed = source;
    break;
  } catch (error) {
    console.warn('Three.js source failed:', source, error);
  }
}
...
if (!THREE) {
  if (loadNoticeEarly) {
    ...
    loadNoticeEarly.innerHTML =
      '<strong>3D engine could not load.</strong><span>Please check the connection and reload the page.</span>';
