// V15.6.6 classroom presentation refinement + final aquarium audio
...
// V15.6.6 event-only gurgle --------------------------------------------------
// Continuous aquarium ambience has been removed completely.
// There is no AudioContext graph, flow sampling or per-frame audio work.
//
// The short local gurgle plays only for two attention-grabbing moments:
//   1) heater Off -> On
//   2) Tutorial start
//
// A small mute toggle is retained because it is useful in a classroom. Sound
// effects are enabled by default, but no audio resource is created until one
// of the two approved user-driven events actually occurs.
...
function playHeaterGurgle() {
  ...
  if (promise?.catch) {
    promise.catch(error => {
      console.warn('Could not play heater gurgle:', error);
    });
  }
  return true;
}

function stopHeaterGurgle() {
  if (!heaterGurgleAudio) return;
  try {
...
function syncSoundUi() {
  ...
  soundToggleBtn.title = soundEnabled
    ? 'Sound effects on — decorative only; not part of the convection physics'
    : 'Sound effects off';
}

function setSoundEnabled(enabled) {
  soundEnabled = Boolean(enabled);
  if (!soundEnabled) stopHeaterGurgle();
  syncSoundUi();
}
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) stopHeaterGurgle();
});
