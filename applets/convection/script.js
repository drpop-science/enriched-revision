...
camera.position.set(10.8, 8.8, 11.8);
...
const TANK = { width: 7.0, height: 7.2, depth: 4.8 };
const BOUNDS = {
...
function flowField(x, y, z, strength) {
  ...
  const vertical = strength * 0.20 * envelope * verticalShape * Math.sin(Math.PI * h);
...
function updateParticles(dt) {
  ...
  const targetCirculation = heat * 0.864 * Math.min(1, elapsed / 5.5);
...
const sampleCubeSize = 1.78;
const warmSamplePos = new THREE.Vector3(heaterX, 2.45, 0);
const coolSamplePos = new THREE.Vector3(2.15, 1.12, 0.35);
...
function updateSamplePositions() {
  ...
  warmSamplePos.set(safeWarmX, 2.45, 0);
  ...
  const coolX = coolSampleSide * (BOUNDS.xMax - half - 0.20);
  coolSamplePos.set(coolX, 1.12, 0.35);
...
function updateSampleCounterPositions() {
  ...
  projectCounter(warmCounter3d, warmSamplePos.clone().add(new THREE.Vector3(0, 1.02, 0)));
  projectCounter(coolCounter3d, coolSamplePos.clone().add(new THREE.Vector3(0, 1.02, 0)));
}
