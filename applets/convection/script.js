// V15.6.5 classroom presentation refinement + final aquarium audio
...
const soundDisclaimerToast = document.querySelector('#soundDisclaimerToast');
...
// V15.6.5 final aquarium audio ---------------------------------------------
// Sound is presentation atmosphere, not physics. The audio is intentionally
// simple and cheap: two pre-generated local WAV assets, no procedural noise,
// no oscillator spawning and no per-frame filter/gain sonification.
//
// 1) aquarium-atmosphere.wav loops at the exact approved 1x level whenever
//    the solved fluid field contains a meaningful convection current.
//    It is either ON at unity gain or OFF; there is no flow-based attenuation.
// 2) heater-gurgle.wav plays only when the heater changes Off -> On and once
//    at the start of Tutorial.
//
// The flow-presence check is deliberately sparse (~3 Hz) and samples only a
// small subset of the already-solved velocity grid, keeping audio overhead tiny.
const AudioContextClass = window.AudioContext || window.webkitAudioContext;
let soundEnabled = false;
let soundEngine = null;
let soundInitialising = false;
let soundSuspendTimer = null;
let soundDisclaimerShown = false;
let soundToastTimer = null;
let lastFlowAudioCheckMs = -Infinity;
let ambienceFlowActive = false;
const FLOW_AUDIO_CHECK_MS = 340;
const FLOW_AUDIO_ON_THRESHOLD = 0.018;
const FLOW_AUDIO_OFF_THRESHOLD = 0.012;
const HEATER_GURGLE_URL = 'assets/heater-gurgle.wav';
const AQUARIUM_AMBIENCE_URL = 'assets/aquarium-atmosphere.wav';
...
async function ensureSoundEngine() {
  ...
  if (soundInitialising || !AudioContextClass) return false;

  soundInitialising = true;
  try {
    const ctx = new AudioContextClass();
    if (ctx.state !== 'running') await ctx.resume();

    const [ambienceBuffer, gurgleBuffer] = await Promise.all([
      fetchAudioBuffer(ctx, AQUARIUM_AMBIENCE_URL),
      fetchAudioBuffer(ctx, HEATER_GURGLE_URL),
    ]);
    ...
    // Unity gain = the exact 1x level of the approved audio asset.
    ...
    const gurgleBus = ctx.createGain();
    // Preserve the V15.6.4 gurgle playback level the user approved.
    gurgleBus.gain.value = 0.74;
    gurgleBus.connect(master);
    ...
    console.warn('Could not initialise aquarium sound effect:', error);
    return false;
  } finally {
    soundInitialising = false;
  }
}

function setAudioTarget(param, value, seconds = 0.08) {
  if (!soundEngine || !param) return;
  param.setTargetAtTime(value, soundEngine.ctx.currentTime, seconds);
}

function showSoundDisclaimerOnce() {
  if (soundDisclaimerShown || !soundDisclaimerToast) return;
  soundDisclaimerShown = true;

  clearTimeout(soundToastTimer);
  soundDisclaimerToast.hidden = false;
  soundDisclaimerToast.classList.remove('show');
  void soundDisclaimerToast.offsetWidth;
  soundDisclaimerToast.classList.add('show');

  soundToastTimer = setTimeout(() => {
    soundDisclaimerToast.classList.remove('show');
    soundDisclaimerToast.hidden = true;
  }, 5250);
}

function playHeaterGurgle() {
  if (!soundEnabled || !soundEngine || !heaterOn) return false;

  const { ctx, gurgleBuffer, gurgleBus } = soundEngine;
  const source = ctx.createBufferSource();
  source.buffer = gurgleBuffer;

  // Keep the subtle per-play variation from the approved build.
  source.playbackRate.value = 0.97 + Math.random() * 0.06;
  const localGain = ctx.createGain();
  localGain.gain.value = 0.86 + Math.random() * 0.10;

  source.connect(localGain);
  localGain.connect(gurgleBus);
  source.start(ctx.currentTime + 0.015);
...
function sampledConvectionSpeed() {
  // Sparse RMS velocity sample. About 120 cells rather than the full grid.
  let sumSq = 0;
  let count = 0;
  for (let j = 4; j <= FLUID_NY - 3; j += 6) {
    for (let i = 4; i <= FLUID_NX - 3; i += 7) {
      const id = fluidIdx(i, j);
      const u = fluid.u[id];
      const v = fluid.v[id];
      sumSq += u*u + v*v;
...
function updateConvectionAmbience(nowMs = performance.now(), forceCheck = false) {
  ...
  if (!eligible) {
    ambienceFlowActive = false;
    setAudioTarget(soundEngine.ambienceGain.gain, 0, 0.10);
    setAudioTarget(soundEngine.master.gain, soundEnabled ? 1 : 0, 0.08);
    return;
  }

  setAudioTarget(soundEngine.master.gain, 1, 0.08);

  if (!forceCheck && nowMs - lastFlowAudioCheckMs < FLOW_AUDIO_CHECK_MS) return;
  lastFlowAudioCheckMs = nowMs;

  const speed = sampledConvectionSpeed();
  if (ambienceFlowActive) {
    if (speed < FLOW_AUDIO_OFF_THRESHOLD) ambienceFlowActive = false;
  } else if (speed >= FLOW_AUDIO_ON_THRESHOLD) {
    ambienceFlowActive = true;
  }

  // Binary at the approved level: 1x or silent. The short transition only
  // prevents clicks and is not used to encode flow strength.
  setAudioTarget(soundEngine.ambienceGain.gain, ambienceFlowActive ? 1 : 0, 0.10);
}
...
function syncSoundUi() {
  ...
  if (!AudioContextClass) {
    ...
    soundToggleBtn.setAttribute('aria-label', 'Aquarium sound effect is not supported in this browser');
    soundToggleBtn.title = 'Aquarium sound effect is not supported in this browser';
    return;
  }

  soundToggleBtn.disabled = soundInitialising;
  soundToggleBtn.setAttribute(
    'aria-label',
    soundEnabled ? 'Turn off aquarium sound effect' : 'Turn on aquarium sound effect'
  );
  soundToggleBtn.title = soundEnabled
    ? 'Sound on — aquarium atmosphere only; not part of the convection physics'
    : 'Sound off — optional aquarium atmosphere';
}
...
async function setSoundEnabled(enabled) {
  ...
  if (enabled) {
    soundToggleBtn.disabled = true;
    soundToggleBtn.title = 'Loading sound…';

    const ready = await ensureSoundEngine();
    if (!ready) {
      soundEnabled = false;
      syncSoundUi();
      return;
    ...
    soundEnabled = true;
    syncSoundUi();
    showSoundDisclaimerOnce();

    try {
      if (soundEngine.ctx.state !== 'running') await soundEngine.ctx.resume();
    } catch (error) {
      console.warn('Could not resume aquarium sound effect:', error);
    }

    // Do NOT gurgle merely because Sound was enabled. The transient is reserved
    // for the two approved events: heater Off->On and Tutorial start.
    updateConvectionAmbience(performance.now(), true);
    return;
  }
  ...
  if (soundEngine) {
    setAudioTarget(soundEngine.ambienceGain.gain, 0, 0.06);
    setAudioTarget(soundEngine.master.gain, 0, 0.06);
...
heaterOnBtn.addEventListener('click', () => {
  const wasOff = !heaterOn;
  heaterOn = true;
  updateHeaterControls();
  if (wasOff) playHeaterGurgle();
});
