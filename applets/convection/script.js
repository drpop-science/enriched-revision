// V15.7.1 classroom presentation refinement + final aquarium audio
...
// V15.7.1 heater interaction sound system -----------------------------------
// Generic button hover/click sounds have been removed.
//
// The remaining sounds are deliberately tied to the heater itself:
//   - ~3 s gurgle: heater Off -> On and Tutorial start
//   - soft glide/swipe: while the heater is being directly dragged or while
//     the position slider is being swiped.
//
// Event-driven only: no continuous ambience, no solver sampling and no
// per-frame audio work. One Sound toggle controls both heater effects.
let soundEnabled = true;
let heaterGurgleAudio = null;
let heaterSlideAudio = null;
let soundDisclaimerShown = false;
let soundToastTimer = null;

const HEATER_GURGLE_URL = 'assets/heater-gurgle.wav';
const HEATER_SLIDE_URL = 'assets/heater-slide.wav';

function ensureHeaterGurgleAudio() {
  if (heaterGurgleAudio) return heaterGurgleAudio;

  heaterGurgleAudio = new Audio(HEATER_GURGLE_URL);
  heaterGurgleAudio.preload = 'auto';
  heaterGurgleAudio.volume = 1.0;
  return heaterGurgleAudio;
}

function ensureHeaterSlideAudio() {
  if (heaterSlideAudio) return heaterSlideAudio;

  heaterSlideAudio = new Audio(HEATER_SLIDE_URL);
  heaterSlideAudio.preload = 'auto';
  heaterSlideAudio.volume = 0.72;
  return heaterSlideAudio;
}
...
function playHeaterGurgle() {
  if (!soundEnabled || !heaterOn || document.hidden) return false;

  const audio = ensureHeaterGurgleAudio();
...
function startHeaterSlideSound() {
  if (!soundEnabled || document.hidden) return false;

  const audio = ensureHeaterSlideAudio();
...
function stopHeaterSlideSound() {
  if (!heaterSlideAudio) return;

  try {
    heaterSlideAudio.pause();
    heaterSlideAudio.currentTime = 0;
  } catch {}
}

function stopAllSound() {
  if (heaterGurgleAudio) {
    try {
      heaterGurgleAudio.pause();
      heaterGurgleAudio.currentTime = 0;
    } catch {}
  }

  stopHeaterSlideSound();
}
...
function syncSoundUi() {
  ...
  soundToggleBtn.setAttribute(
    'aria-label',
    soundEnabled ? 'Turn off heater sound effects' : 'Turn on heater sound effects'
  );
  soundToggleBtn.title = soundEnabled
    ? 'Sound effects on — heater gurgle + movement sound'
    : 'Sound effects off';
}
...
window.addEventListener('pointerup', () => {
  if (!draggingHeater) stopHeaterSlideSound();
}, { passive: true });

window.addEventListener('pointercancel', stopHeaterSlideSound, { passive: true });
...
heaterPosition.addEventListener('pointerup', stopHeaterSlideSound);
heaterPosition.addEventListener('pointercancel', stopHeaterSlideSound);
heaterPosition.addEventListener('change', stopHeaterSlideSound);
