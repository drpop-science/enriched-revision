...
const counterToggle = document.querySelector('#counterToggle');
const counterToggleRow = document.querySelector('#counterToggleRow');
...
camera.position.set(11.2, 5.4, 6.8);
...
const TANK = { width: 9.2, height: 4.10, depth: 1.18 };
const BOUNDS = {
...
const DEPTH_MOTION_SCALE = 0.38;
const HEAT_EFFECT_RADIUS = HEATER_RADIUS * 1.04;
...
function flowField(x, y, z, strength) {
  // V8: broadened closed circulation cell. The overall strength is raised by
...
function applySoftWallForces(p, dt) {
  // The narrow z dimension represents a thin slice of liquid, not a corridor
  // in which particles should repeatedly ping-pong. A soft repulsive layer
  // turns particles before they reach the glass and helps prevent wall-sticking.
  const xMargin = 0.18;
  const zMargin = 0.14;
  const bottomMargin = 0.14;
  const wallK = 8.0;
  const depthWallK = 10.0;

  if (p.x < BOUNDS.xMin + xMargin) p.vx += wallK * (BOUNDS.xMin + xMargin - p.x) * dt;
  if (p.x > BOUNDS.xMax - xMargin) p.vx -= wallK * (p.x - (BOUNDS.xMax - xMargin)) * dt;
...
function updateParticles(dt) {
  ...
  for (const p of particles) {
    ...
    const verticalHeat = Math.exp(-Math.pow((p.y - 0.32) / 0.50, 2));
...
const sampleBox = { width: 1.65, height: 1.40, depth: 0.82 };
const sampleCubeSize = sampleBox.width;
const warmSamplePos = new THREE.Vector3(heaterX, 1.68, 0);
const coolSamplePos = new THREE.Vector3(3.25, 0.92, 0.25);
...
function updateSamplePositions() {
  ...
  coolSamplePos.set(coolX, 0.92, 0.25);
...
function sampleActualParticles(pos) {
  const halfX = sampleBox.width / 2;
  const halfY = sampleBox.height / 2;
  const halfZ = sampleBox.depth / 2;
  let count = 0;
  let totalTemp = 0;
  for (const p of particles) {
    if (
      Math.abs(p.x - pos.x) <= halfX &&
      Math.abs(p.y - pos.y) <= halfY &&
      Math.abs(p.z - pos.z) <= halfZ
    ) {
      count++;
      totalTemp += p.temp;
    }
...
function updateDensitySamples() {
  ...
  const sampleVolume = sampleBox.width * sampleBox.height * sampleBox.depth;
...
function updateSampleCounterPositions() {
  if (!densityToggle.checked || !counterToggle.checked) return;
  projectCounter(warmCounter3d, warmSamplePos.clone().add(new THREE.Vector3(0, 0.90, 0)));
  projectCounter(coolCounter3d, coolSamplePos.clone().add(new THREE.Vector3(0, 0.90, 0)));
}

function syncDensityUi() {
  const compareOn = densityToggle.checked;
  const countersOn = compareOn && counterToggle.checked;

  sampleGroup.visible = compareOn;
  densityPanel.hidden = !compareOn;
  counterToggle.disabled = !compareOn;
  counterToggleRow.classList.toggle('disabled-row', !compareOn);
  densityGrid.hidden = !countersOn;
  warmCounter3d.hidden = !countersOn;
  coolCounter3d.hidden = !countersOn;
...
const walkthroughSteps = [
  ...
  },
  {
    ...
    text: 'The warmed liquid expands slightly: its particles are, on average, a little farther apart. The same volume therefore contains less mass, so the liquid is less dense.',
    view: () => ({
  ...
  },
  {
    ...
    text: 'Farther from the heater, particles move less vigorously and become slightly more closely packed. The cooler liquid is denser, so it sinks and flows back underneath, completing the convection current.',
    view: () => ({
      position: new THREE.Vector3(11.2, 5.4, 6.8),
...
function setWalkthroughOverlay(mode) {
  ...
  warmCounter3d.hidden = true;
  coolCounter3d.hidden = true;
}
...
function startWalkthrough() {
  ...
  walkthroughSavedState = {
    ...
    counters: counterToggle.checked,
...
function finishWalkthrough(restoreView = false) {
  ...
  if (walkthroughSavedState) {
    ...
    counterToggle.checked = walkthroughSavedState.counters;
...
densityToggle.addEventListener('change', () => {
  if (!densityToggle.checked) counterToggle.checked = false;
  syncDensityUi();
});
...
function animate(now) {
  ...
  if (isRunning) {
    ...
    updateParticles(dt);
    if (densityToggle.checked && counterToggle.checked) {
      updateDensitySamples();
      densityAccumulator = 0;
    }
    updateNarration();
